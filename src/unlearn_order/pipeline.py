...
def run_pipeline(
    model, tokenizer, experiments, batch_size=16, tolerance=0.05
):
    for (t, name, dataset) in experiments:
        ...
        if t == "f" or t == "u":
            ...
            model, loss_traj, acc_tracj = finetune_model(
                model, tokenizer, dataset, batch_size=batch_size, shuffle_labels=shuffle_labels, tolerance=tolerance
            )
